...
/*void drawWorld (unsigned short *currentWorld, unsigned short *newWorld, SDL_Renderer* renderer,int firstRow,int lastRow,int worldWidth, int worldHeight){
...
void pintaWorld(unsigned short *currentWorld, SDL_Renderer *renderer, int firstRow, int lastRow, int worldWidth)
{
    ...
    for (int col = 0; col < worldWidth; col++)
    {
        for (int row = firstRow; row <= lastRow; row++)
        {
            ...
            switch (getCellAtWorld(&cell, currentWorld, worldWidth))
            {
				case CELL_NEW:
					drawCell(&cell, CELL_NEW, renderer);
					break;
				...
				case CELL_EMPTY:
					drawCell(&cell, CELL_EMPTY, renderer);
					break;
